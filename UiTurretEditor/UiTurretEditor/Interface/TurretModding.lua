...
Remade/Updated By GloGang3Hunna <thenoahcrandall@gmail.com>
...
function Win:BuildUI()
	...
	self.BtnHeat.tooltip = "[beta] Reduce the heat generated per shot, improve cooldown rate per second."
	...
	self.BtnMaxHeat.tooltip = "[beta] Improve mounting heat dissipation."
	...
	self.BtnDamage.tooltip = "[beta] Increase the firepower."
	...
	self.BtnEfficiency.tooltip = "[beta] Increase the efficiency of mining and scav lasers."
	...
	self.BtnSpeed.tooltip = "[beta] Increase the tracking speed."
	...
	self.BtnRange.tooltip = "[beta] Increase the range."
	...
	self.BtnFireRate.tooltip = "[beta] Increase the fire rate."
	...
	self.BtnAccuracy.tooltip = "[beta] Increase the accuracy."
	...
	self.BtnProjectileSpeed.tooltip = "[beta] Increase the projectile velocity."
	...
	self.BtnTargeting.tooltip = "[beta] Toggle Automatic Targeting.\n(Does not consume turrets)"
	...
	self.BtnCoaxial.tooltip = "[beta] Toggle Coaxial Mounting.\n(Does not consume turrets)"
	...
	self.BtnSize.tooltip = "[beta] Adjust Turret Size.\n(Does not consume turrets)"
	...
	self.BtnColour.tooltip = "[beta] Set Weapon Color.\n(Does not consume turrets)"
	...
	self.BtnMkFlak.tooltip = "[beta] Convert an Anti-Fighter turret into a flak barrier turret. Requries scrapping " .. Config.FlakCountRequirement .. " other Anti-Fighter turrets for parts."

	--if(Config.Experimental) then
		self.BtnMkCool = self.Window:createButton(
			Rect(),
			"[beta] Liquid Naonite Cooling System",
		...
		self.BtnMkCool.tooltip = "[beta] Apply a Liquid Naonite Cooling System to this turret."
	--end
...
function Win:UpdateFields()
	...
	self.BtnTargeting.caption = "[beta] Targeting (Cr. " .. toReadableValue(Config.CostTargeting) .. ")"
	self.BtnTargeting.tooltip = "[beta] Toggle Automatic Targeting.\n(Does not consume turrets)"
	self.BtnCoaxial.caption = "[beta] Coaxial (Cr. " .. toReadableValue(Config.CostCoaxial) .. ")"
	self.BtnColour.caption = "[beta] Colour HSV (Cr. " .. toReadableValue(Config.CostColour) .. ")"
	self.BtnSize.caption = "[beta] Scale (Cr. " .. toReadableValue(Config.CostSize) .. ")"

	self.BtnHeat.caption = "[beta] Heat Sinks"
	...
	self.BtnMounting.caption = "[beta] Reinforced Mount"
	...
	if(FixTargetingNerfEnable) then
		...
		self.BtnTargeting.tooltip = "[beta] Fixes automatic targeting nerf by removing targeting and fixing the damage values. You must then re-apply targeting afterwards."
	end
...
function Win:UpdateBinLabel()
	...
	if(BuffValue > 0) then
		self.BinLabel.caption = "[beta] Turrets To Scrap (+" .. round(BuffValue,3) .. "%)"
	else
		self.BinLabel.caption = "[beta] Turrets To Scrap"
	end
...
-- raise heat radiation
	...
	if(Mock == nil) then
		PrintError("[beta] No turret selected")
		return
	end

	if(BuffValue == 0.0) then
		PrintError("[beta] No turrets in scrap bin")
		return
	end

	if(TurretLib:GetWeaponHeatRate(Real) == 0) then
		PrintWarning("[beta] This turret is not producing any heat.")
		return
	end
...
-- lower power requirement.
	...
	if(TurretLib:GetWeaponMaxHeat(Real) == 0) then
		PrintWarning("[beta] This turret does not take any heat.")
		return
	end
...
-- raise fire rate.
	...
	if(TurretLib:GetWeaponFireRate(Real) == 0) then
		PrintWarning("[beta] This turret does not have a fire rate")
		return
	end
...
-- raise turret speed.
	...
	if(TurretLib:GetWeaponSpeed(Real) == 0) then
		PrintWarning("[beta] This turret does not turn apparently")
		return
	end
...
-- raise weapon range.
	...
	if(TurretLib:GetWeaponRange(Real) == 0) then
		PrintWarning("[beta] This turret has no reach apparently")
		return
	end
...
-- raise weapon damage
	...
	if(TurretLib:GetWeaponDamage(Real) == 0) then
		PrintWarning("[beta] This turret does not do any damage")
		return
	end
...
-- raise accuracy
	...
	if(CurrentValue == 0) then
		PrintWarning("[beta] This turret has no accuracy apparently")
		return
	end

	if(CurrentValue == 1) then
		PrintWarning("[beta] This turret is at max accuracy.")
		return
	end
...
-- raise rifficiency
	...
	if(CurrentValue == 0) then
		PrintWarning("[beta] This turret has no efficiency apparently")
		return
	end

	if(CurrentValue == 1) then
		PrintWarning("[beta] This turret is at max efficiency.")
		return
	end
...
-- lower mounting cost
	...
	if(CurrentValue <= Config.TurretSlotMin) then
		PrintError("[beta] This turret is already at minimum slot use.")
		return
	end

	if(not self:ShouldAllowMountingUpgrade(Real)) then
		PrintError("[beta] Scrapping requirements not met for upgrading the mounting.")
		return
	end
...
-- toggle targeting
	...
	if(PlayerRef.money < Config.CostTargeting) then
		PrintError("[beta] You do not have enough credits")
		return
	end
...
-- change turrent size
	...
	if(not self:ShouldAllowFlakConversion(Real)) then
		PrintError("[beta] Scrapping requirements not met for flak cannon conversion.")
		return
	end
...
-- raise weapon damage
	...
	if(TurretLib:GetWeaponProjectileSpeed(Real) == nil) then
		PrintWarning("[beta] This turret instantly slaps your mom.")
		return
	end
